...
_st(canvas)._path_(_st(_st(_st(_st(_st(_st(_st("M").__comma(x1)).__comma(" ")).__comma(y1)).__comma("L")).__comma(x2)).__comma(" ")).__comma(y2));
return self}, function($ctx1) {$ctx1.fill(self,"drawOn:for:",{canvas:canvas,anEdge:anEdge,x1:x1,y1:y1,x2:x2,y2:y2},smalltalk.ROLineShape)});},
args: ["canvas", "anEdge"],
source: "drawOn: canvas for: anEdge\x0a\x09| x1 y1 x2 y2 |\x0a\x09x1 := anEdge from position x.\x0a\x09y1 := anEdge from position y.\x0a\x0a\x09x2 := anEdge to position x.\x0a\x09y2 := anEdge to position y.\x0a\x0a\x09canvas path: 'M', x1,' ', y1, 'L', x2, ' ', y2 \x0a\x0a\x22\x09canvas path: 'M10 10L90 90'\x0a\x22\x0a\x22\x09canvas \x0a\x09\x09rect: (anElement position x)\x0a\x09\x09with: (anElement position y) \x0a\x09\x09with: (self widthFor: anElement) \x0a\x09\x09with: (self heightFor: anElement) .\x0a\x22",
